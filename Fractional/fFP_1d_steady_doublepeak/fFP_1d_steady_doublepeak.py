...
# ============================================================================
# POTENTIAL & DRIFT  (double-well: V(x) = x^4 - x^2, b = -V')
# ============================================================================
def drift_field(x):
    """b(x) = -4x(x^2 - 1)  (gradient of V = x^4 - x^2)"""
...
def run_particle_benchmark():
    print("\nRunning particle simulation benchmark (double-well)...")
    ...
    for step in range(n_steps):
        ...
        if (step + 1) % 500 == 0:
            print(f"  step {step+1}/{n_steps}, median={np.median(particles):.3f}")
...
def plot_results(generator, particle_samples, history):
    ...
    ax.set_title(f'Steady-State 1D Double-Peak (α={ALPHA})', fontsize=13)
    ax.legend()
    ...
    plt.suptitle(f'Fractional Steady-State Double-Well  (α={ALPHA})',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'steady_doublepeak_comparison.png'),
...
def main():
    print("=" * 70)
    print("Fractional Steady-State 1D Double-Peak  (Section 4.2 variant)")
    print("=" * 70)
