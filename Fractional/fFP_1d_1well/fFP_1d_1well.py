...
D_BASE = 5  # Base distribution dimension (can be > D for expressivity)

# Square (quadratic) potential well parameters
WELL_STRENGTH = 1.0  # Coefficient for V(x) = (1/2) * k * x^2

# Initial condition: Gaussian slightly offset from well bottom
IC_CENTER = 1.0  # Offset from well center at x=0
IC_SIGMA = 0.3
...
# Diffusion scaling for fractional Laplacian
# For the equation: ∂ρ/∂t + (−Δ)^(α/2) ρ + ∇·(bρ) = 0
# The normalization constant relates the fractional Laplacian to the Lévy process
# Standard choice: use σ such that the characteristic function matches
# For α-stable with scale σ: E[exp(ikX)] = exp(-σ^α|k|^α)
# This gives σ = (2*Γ(1-α/2)*cos(πα/4))^(-1/α) for symmetric stable
# For computational simplicity, we use σ = 1 and adjust if needed
LEVY_SCALE_FACTOR = 1.0  # Adjustable prefactor for Lévy scale
...
def square_well_potential(x):
    """Quadratic (harmonic) potential well: V(x) = (1/2) * k * x^2"""
    # x shape: (batch, 1)
    return 0.5 * WELL_STRENGTH * x ** 2


def drift_field(t, x):
    """Drift b(t,x) = -dV/dx = -k*x (linear restoring force)"""
    # For quadratic potential V = (1/2)*k*x^2, gradient is dV/dx = k*x
    # So drift b = -dV/dx = -k*x
    return -WELL_STRENGTH * x
...
def train():
    ...
    print(f"Fractional order α: {ALPHA}")
    print(f"Quadratic potential well: V(x) = ½kx², k={WELL_STRENGTH}")
    print(f"Drift: b(x) = -kx = {-WELL_STRENGTH}x")
...
def visualize_time_evolution(generator, particle_trajectories):
    ...
    for idx, t in enumerate(time_steps):
        ...
        # Add potential well visualization (quadratic)
        x_plot = np.linspace(X_RANGE[0], X_RANGE[1], 200)
        V_plot = 0.5 * WELL_STRENGTH * x_plot ** 2
        ax2 = ax.twinx()
...
def plot_potential_and_initial_condition():
    ...
    # Plot potential
    ax1.plot(x, V, 'r-', linewidth=2, label='Potential V(x) = ½kx²')
    ...
    plt.title(f'Quadratic Potential Well and Initial Condition\n' +
              f'α={ALPHA}, k={WELL_STRENGTH}, IC center={IC_CENTER}',
